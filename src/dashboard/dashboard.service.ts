...
export class DashboardService {
  ...
  async getVolunteerStats(userId: number) {
    const [
      ...
      this.programCenterRepository.findOne({
  ...
  private async getRecentActivities(): Promise<Activity[]> {
    ...
    activities.push(...recentUploads.map(upload => ({
    ...
    activities.push(...recentAttendance.map(record => ({
  ...
  private async getVolunteerRecentActivities(userId: number): Promise<Activity[]> {
    const activities: Activity[] = [];
